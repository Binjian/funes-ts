...
def gen_vol_data(data_len,seq_len,mode=43):
...
# fake_label= [2 for _ in range(fake_data.shape[0])]
# labels = np.array(train_label + fake_label)

#####  sine triangle square  #####
# sine_data = sine_data_generation(100,50,1)
# sine_label = [5 for _ in range(sine_data.shape[0])]
# triangle_data = triangle_wave_data(100,50,1)
# triangle_label = [5 for _ in range(triangle_data.shape[0])]
# # square_data = square_wave_data(1000,50,1)
# # square_label = [0 for _ in range(square_data.shape[0])]
# # vol_data = np.load('../datautils/vol_gen_data_50.npy')[-50:]
# vol_data = gen_vol_data(100,50,mode=39)
# vol_label = [0 for _ in range(vol_data.shape[0])]
# data = np.concatenate((sine_data,triangle_data),axis=0)
# labels = np.array(sine_label + triangle_label)
# # data = fake_data[:50]
# # labels = np.array(fake_label[:50])
...
for m in range(len(multimodal_list)):
    ...
    vol_data = gen_vol_data(3000,50,mode=multimodal_list[m])
    if m == 0:
        data = vol_data
    else:
...
# data = np.concatenate((data,sine_data,triangle_data),axis=0)
# labels = np.array(labels+sine_label + triangle_label)
...
#                         learning_rate_init=0.001,max_iter=500,verbose=True,early_stopping=False)
#     clf.fit(X_train,Y_train)
#     y_pred = clf.predict(X_test)
#     y_true = list(Y_test)
#     # print(cls.score(X_test,Y_test))
#     print(classification_report(y_true,y_pred))
#     # with open('../../data/output/kats_output/skl_cls_vol_600epoch.pickle', 'wb') as f:
#     #     pickle.dump(clf,f)
#
...
class KATS_Classifier(torch.nn.Module):
    def __init__(self,input_dim,hidden_dim,class_num):
        ...
        self.batch_norm2 = torch.nn.BatchNorm1d(int(hidden_dim/2))
        # self.out = torch.nn.Linear(int(hidden_dim/2), 1)
        self.out = torch.nn.Linear(int(hidden_dim / 2), class_num)
        # self.softmax = torch.nn.Softmax()
        self.relu = torch.nn.ReLU()


    def forward(self,X):
        h1 = self.batch_norm1(self.linear1(X))
        h2 = self.relu(self.batch_norm2(self.linear2(h1)))
...
def evaluator(model,dataloader):
    ...
    for x, y in dataloader:
        ...
        logit += list(torch.max(torch.softmax(out,dim=1),dim=1).values.cpu().detach().numpy())
        label += list(torch.max(torch.softmax(out, dim=1), dim=1).indices.cpu().detach().numpy())
...
def train(model,train_dataloader,val_dataloader,EPOCH,lr,save_path):
    ...
    writer = SummaryWriter(log_dir='../../data/output/kats_output/log')
    for epoch in range(EPOCH):
        ...
        writer.add_scalar(tag='TrainLoss', scalar_value=sum(loss) / len(loss), global_step=epoch)
        writer.add_scalars(main_tag='Metrics', tag_scalar_dict={'ValLoss': sum(val_loss)/len(val_loss)}, global_step=epoch)
...
batch_size = 256
lr = 0.001
save_path = f'../../data/output/kats_output/multiclass/kats_clf_multimodal_vol_{EPOCH}_{batch_size}_{hidden_dim}_{lr}_{datetime.datetime.now().strftime("%Y%m%d-%H%M%S")}.params'

clf = KATS_Classifier(input_dim,hidden_dim,class_num)
clf.to('cuda')
...
# clf.load_state_dict(torch.load('../../data/output/kats_output/multiclass/kats_clf_multimodal_vol_1000_256_32_0.001_20221110-104708.params'))
# evaluator(clf,dataSet_loader)
