...
def mixed_visualization(ori_data, analysis, out_list=None, generated_data=None):
    ...
    ori = np.load('../../datautils/sine+trix2_data.npy').squeeze(axis=-1)  ##############
...
def VOL_cross_correlation():
    ...
    for j in range(1, pic_num + 1):
        ...
        for i in range(fft_ori_data.shape[0]):
            ...
            cor = signal.correlate(MinMaxScaler(train_data[i, :train_time[i], 0]),
                                   MinMaxScaler(fake_data[j, :fake_time[j], 0]))  ##########################
        ...
        min_y = min(min(fake_data[j, :fake_time[j], 0]),
                    min(train_data[cor_list.index(max(cor_list)), :train_time[cor_list.index(max(cor_list))], 0]))
        max_y = max(max(fake_data[j, :fake_time[j], 0]),
                    max(train_data[cor_list.index(max(cor_list)), :train_time[cor_list.index(max(cor_list))], 0]))
...
def mixed_wave():
    data_path = '../../../data/output/sine+trix2_1026/MIX-20221026-092308/mdl/MIX-vanilla-1000-1000-1000-64-24-3-0.001-50-4000'
    # MIX-20221022-103412/mdl/MIX-vanilla-1000-1000-1000-128-24-2-0.001-50-4000'
    # MIX-20221022-234805/mdl/MIX-vanilla-1000-1000-1000-64-24-3-0.001-50-4000' best

    '''
    NAN
    SQU-20221014-172019  SQU-vanilla-1000-1000-1000-64-16-2-0.001-50-4000
    SQU-20221015-024938  SQU-vanilla-1000-1000-1000-128-8-2-0.001-50-4000

    SQU-20221018-181705/mdl/SQU-vanilla-1000-1000-1000-64-8-2-0.001-50-4000/ckp.pt
    SQU-20221018-225240/mdl/SQU-vanilla-1000-1000-1000-64-24-2-0.001-50-4000/ckp.pt
    '''
...
def single_wave():
    data_path = '/home/veos/devel/funes/data/output/squ_1021_+0.5/SQU-20221023-184122/mdl/SQU-vanilla-1000-1000-1000-128-32-3-0.001-50-4000'
...
def VOL():
    data_path = '../../../data/output/bd_vol_1014/VOL-20221017-012444/mdl/VOL-vanilla-1000-1000-1000-128-32-3-0.001-50-4000'
...
if __name__ == "__main__":
    ...
    #
    # y = np.linspace(0.5*np.pi,1.5*np.pi,30)
    # print(len(x),len(y))
    # print(np.sin(x).shape,np.sin(y).shape)
    # cor_list = []
    # max_idx_list = []
    # for i in range(0,5):
    #     # x = np.linspace(0, 2 * np.pi, 60)
    #
    #     cor = signal.correlate(np.sin(x+i),np.sin(y))
    #
    #     # print(len(cor))
    #     # print(cor.argmax())
    #     cor_list.append(max(cor))
    #     max_idx_list.append(cor.argmax())
    #     print(i,max(cor))
    #     # print(cor,cor.argmax())
    #     plt.plot(np.sin(x+i))
    #     plt.show()
    #
    # max_val_idx = cor_list.index(max(cor_list))
    # plt.plot(np.sin(x+max_val_idx))
    #
    # y_ = [_ for _ in range(max_idx_list[max_val_idx]-len(y)+1,max_idx_list[max_val_idx]+1)]
    # print(y_)
    # print(max_val_idx,max_idx_list[max_val_idx],len(x),len(y))
    # # print(len(y_))
    # plt.plot(y_,np.sin(y))
    #
    # plt.show()
    ...
    def test():
        ...
        cor = signal.correlate(np.sin(x), np.sin(y))

        # max_val_idx = cor_list.index(max(cor_list))
        # plt.plot(np.sin(np.linspace(0, max_val_idx * np.pi, 60)))

        plt.plot(np.sin(x))
        plt.plot(np.sin(y))
        y_ = [_ for _ in range(cor.argmax() - 30 + 1, cor.argmax() + 1)]
        print(y_)
        # print(max_idx_list[max_val_idx], 30)
        # print(len(y_))
        plt.plot(y_, np.sin(y))
