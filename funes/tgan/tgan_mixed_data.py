...
def train_eval_cmd(args):
    ...
    # tri_data = np.load('../datautils/sine+tri_40000data.npy')[:20000]
    # sine_data = np.load('../datautils/sine+tri_40000data.npy')[20000:]
    # print(tri_data.shape,sine_data.shape)
    # np.random.shuffle(sine_data)
    # np.random.shuffle(tri_data)
    #
    # if args.data == 'sine':
    #     sine = sine_data[:int(5000*args.split)]
    #     print(args.data,args.split,sine.shape)
    #     tri = tri_data[:5000]
    #     X = np.concatenate((tri,sine),axis=0)
    # elif args.data =='triangle':
    #     sine = sine_data[:5000]
    #     tri = tri_data[:int(5000 * args.split)]
    #     print(args.data, args.split, tri.shape)
    #     X = np.concatenate((sine, tri), axis=0)

    tri_data = triangle_wave_data(5000, 50, 1)
    sine_data = sine_data_generation(5000, 50, 1)
    X = np.concatenate((sine_data, tri_data), axis=0)

    T = np.array([X.shape[1] for _ in range(X.shape[0])])


    # X = np.expand_dims(X, axis=-1)  # vol
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--data", default='sine', type=str)
    ...
    # Output directories
    out_dir = funes.tgan.PROJROOT + "/data/output/sine+tri_1213/" + args.out_subdir
    try:
        os.makedirs(out_dir)
