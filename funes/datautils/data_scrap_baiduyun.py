...
logfilename = (
        "../../data/logs/data_scrap/"
        + datetime.now().strftime("%y-%m-%d-%h-%m-%s_%f")[:-3]
        + ".log")
logging.basicConfig(filename=logfilename,level=logging.ERROR)
logger = logging.getLogger('data scrap')
logger.info(f'===========START=========== {datetime.now().strftime("%y-%m-%d-%h-%m-%s_%f")[:-3]}')
#######################################################################
### data fetch
...
try:
    ...
    for car in car_list:
        ...
        for i in range(cycle):
            ...
            try:
                res = es.options().search(
                    index="tsp-vehicle-log3*", body=body,size=20000, request_timeout=30000
                )
                if i == 0:
        ...
        try:
            ...
            os.remove("../../data/train-data/" + carNo + ".xlsx")
            continue
        writer.save()
        writer.close()
        os.remove("../../data/all-info/" + carNo + ".json")
        os.remove("../../data/filt-data/" + carNo + ".xlsx")

except Exception as e:
    logger.info(datetime.now().strftime("%y-%m-%d-%h-%m-%s_%f")[:-3])
    logger.info(traceback.format_exc())
logger.info(f'===========END=========== {datetime.now().strftime("%y-%m-%d-%h-%m-%s_%f")[:-3]}')
