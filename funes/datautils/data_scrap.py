...
try:
    ...
    for car in car_list:
        ...
        # time range
        start_time = "2022.8.1 00:00:00"   # 6.10-8.1
        end_time = "2022.8.22 00:00:00"
        ...
        if not os.path.exists("../../data/filt-data/" + datetime.now().strftime("%Y-%m-%d")+'/'):
            os.mkdir("../../data/filt-data/" + datetime.now().strftime("%Y-%m-%d")+'/')

        wb.save("../../data/filt-data/" + datetime.now().strftime("%Y-%m-%d") +'/'
                    + carNo + ".xlsx")
        ...
        wb = pd.read_excel(
            "../../data/filt-data/" + datetime.now().strftime("%Y-%m-%d") + '/'
            + carNo + ".xlsx",
            sheet_name=carNo,
        )
        ...
        ocv = []
        timestamp = []
        if not os.path.exists("../../data/train-data/" +datetime.now().strftime("%Y-%m-%d") +'/'):
            os.mkdir("../../data/train-data/" +datetime.now().strftime("%Y-%m-%d") +'/')
        writer = pd.ExcelWriter("../../data/train-data/" +datetime.now().strftime("%Y-%m-%d") +'/'
                                + carNo + ".xlsx")
        # writer = pd.ExcelWriter("../../data/train-data/" + carNo + ".xlsx")
        sheet0 = pd.DataFrame()
        ...
        sheet0.to_excel(writer,index=False,sheet_name='meta-info')
        try:
            for i in range(198):
                ...
                tsdf = pd.DataFrame()
                tsdf["timestamp"] = np.float64(timestamp)  # 32bytes
                # tsdf['vol_num'] = vol_num
                tsdf["current"] = np.float64(cur)
                tsdf["vol"] = np.float64(vol)
                tsdf["soc"] = np.float64(soc)
                tsdf["ocv"] = np.float64(ocv)
                tsdf.to_excel(writer,index=False,sheet_name='Cell_'+str(i+1))
        except IndexError as e:
            print(IndexError,e)
            print(f'drop {carNo}.xlsx')
            os.remove("../../data/train-data/" + datetime.now().strftime("%Y-%m-%d") +'/' + carNo + ".xlsx")
            continue
        writer.save()
        writer.close()
