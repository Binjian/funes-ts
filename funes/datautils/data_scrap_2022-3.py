...
for car in car_list:
    ...
    for i in range(cycle):
        ...
        res = es.search(
            index="tsp-vehicle-log3*", body=body, size=20000, request_timeout=300000
        )
        if i == 0:
    ...
    for i in range(198):
        vol = vol + segdf.iloc[:, i + 4].values.tolist()
        timestamp = timestamp + segdf.iloc[:,0].values.tolist()
        cur = cur + segdf.iloc[:, 1].values.tolist()
        soc = soc + segdf.iloc[:, 2].values.tolist()
        ocv = ocv + segdf.iloc[:, 3].values.tolist()

    tsdf = pd.DataFrame()
    tsdf["timestamp"] = np.float64(timestamp)
    ...
    tsdf.to_csv(
        "../../data/train-data/" + carNo + ".csv",
        mode="w",
        index=False,
        header=["current", "vol", "soc", "ocv"],
        na_rep="nan",
    )
