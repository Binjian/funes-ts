...
class Generator(nn.Module):
    def __init__(self, seq_len=args.seq_len,  # 150
                 patch_size=args.patch_size, # 15
                 channels=1, num_classes=9, latent_dim=args.latent_dim, embed_dim=50, depth=3,  #### channels = feature dim
                 num_heads=5, forward_drop_rate=0, attn_drop_rate=0):   #####   drop 0.5
...
class ClassificationHead(nn.Sequential):
    def __init__(self, emb_size=50, n_classes=2):
        super().__init__()
...
class Discriminator(nn.Sequential):
    def __init__(self, 
                 ...
                 patch_size=args.patch_size, # 15
                 emb_size=50,
                 seq_length = args.seq_len, # 150
                 depth=3, 
                 n_classes=1, 
                 **kwargs):
        super().__init__(
            PatchEmbedding_Linear(in_channels, patch_size, emb_size, seq_length),
            Dis_TransformerEncoder(depth, emb_size=emb_size, drop_p=0, forward_drop_p=0, **kwargs), # drop 0.5
