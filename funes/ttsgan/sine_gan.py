...
from utils.utils import set_log_dir, save_checkpoint, create_logger, save_image
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.load_path:
        ...
        if args.rank == 0:  #######################################
            args.path_helper = set_log_dir(f'../../data/ttsgan_output/{args.exp_name}-1208/', args.exp_name)
...
def gen_plot(gen_net, epoch, class_name):
    ...
    for i in range(10):
        ...
        fake_noise = torch.FloatTensor(np.random.normal(0, 1, (1, 100))).to(f'cuda:{0}')
        fake_sigs = gen_net(fake_noise).to(f'cpu').detach().numpy()
        # fake_sigs = fake_sigs.cpu()

        synthetic_data.append(fake_sigs)
    # np.save('gen.npy',np.array(synthetic_data))
    # print('########',np.array(synthetic_data).shape)
